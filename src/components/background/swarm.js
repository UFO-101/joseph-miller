...
// Define Math.Tau if it doesn't exist
Math.Tau = Math.PI * 2;
...
+(function(root) {
  ...
  +(function(root) {
    ...
    pp.render = function(context) {
      var size = Math.max(2, this.size); // Minimum size of 2 pixels
      context.arc(this.p.x, this.p.y, size, 0, Math.PI * 2, false);
    };
...
export function initSwarm() {
  var rctx = new SmallPRNG(+new Date()),
      ...
      settings = {
        particleNum: 500,  // Restored to original value
        fadeOverlay: true,
        rotateColor: true,
  ...
  +(function render() {
    ...
    for(var i = 0; i < particles.length; i += 1) {
      ...
      if(settings.rotateColor) {
        context.fillStyle = 'hsla(' + hue + ', 75%, 50%, .3)'; // Increased opacity since we're not stacking effects
      } else {
        context.fillStyle = settings.staticColorString;
      }
      context.fill();
      context.closePath();
    }
